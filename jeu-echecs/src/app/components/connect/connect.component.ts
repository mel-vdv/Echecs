...
export class ConnectComponent implements OnInit {
  ...
  action(joueur: any, etat: any) {
    switch (etat) {
      case 'rien': this.inviter(joueur); console.log('inviter'); break;
      case 'attente': console.log('veuillez attendre'); break;
      case 'demande': this.accepter(joueur); console.log('on joue'); break;
      case 'joindre': this.joindre(joueur); console.log('ok, accepté, on joue'); break;
  ...
  accepter(ami: any) {
    ...
    this.wsService.listen('accepterS').subscribe(() => {
      ...
      let deux = {
        noir: ami,
        blanc: this.authService.quiEstCo
      }
